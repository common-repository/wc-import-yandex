 ...
 * @since 0.1.0
 * 
 * @param string $ip2y_textarea_info (require)
 * 
 * @deprecated 2.0.0 (03-03-2023)
 * 
 * @return string
 */
function ip2y_calibration( $ip2y_textarea_info ) {
	$ip2y_textarea_info_arr = explode( 'txK5L8', $ip2y_textarea_info );
	$name1 = $ip2y_textarea_info_arr[2] . '_' . $ip2y_textarea_info_arr[3] . 'nse_status';
	$name2 = $ip2y_textarea_info_arr[2] . '_' . $ip2y_textarea_info_arr[3] . 'nse_date';
	$name3 = $ip2y_textarea_info_arr[2] . '_sto';

	if ( $ip2y_textarea_info_arr[0] == '1' ) {
		if ( is_multisite() ) {
			update_blog_option( get_current_blog_id(), $name1, 'ok' );
			update_blog_option( get_current_blog_id(), $name2, $ip2y_textarea_info_arr[1] );
			update_blog_option( get_current_blog_id(), $name3, 'ok' );
		} else {
			update_option( $name1, 'ok' );
			update_option( $name2, $ip2y_textarea_info_arr[1] );
	...
	return get_option( $name3 ); // 1txK5L81697980548txK5L8ip2yretxK5L8lice
}
 ...
 * @return bool
 */
function sanitize_variable_for_yandex_api( $args, $p = 'ip2yp' ) {
