...
trait IP2Y_T_Common_Get_CatId {
	...
	public function set_category_id( $catid = null ) {
		...
		if ( empty( $catid ) ) {
			$this->add_skip_reason( [ 
				'reason' => __( 'The product has no categories', 'wc-import-yandex' ),
				'post_id' => $this->get_product()->get_id(),
				'file' => 'trait-ip2y-t-common-get-catid.php',
				'line' => __LINE__
			] );
			return '';
		}
	...
	private function database_auto_boot( $catid = null ) {
		new IP2Y_Error_Log( sprintf( 'CABINET № %1$s; %2$s %3$s %4$s; Файл: %5$s; %6$s: %7$s',
			$this->get_feed_id(),
			...
			'trait-ip2y-t-common-get-catid.php',
		...
		if ( is_array( $product_cats ) && count( $product_cats ) ) {
			...
			new IP2Y_Error_Log( sprintf( 'CABINET № %1$s; %2$s %3$s %4$s %5$s; Файл: %6$s; %7$s: %8$s',
				$this->get_feed_id(),
				'WARNING: Для товара $this->get_product()->get_id() =',
				$this->get_product()->get_id(),
