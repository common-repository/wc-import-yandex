...
final class IP2Y_Api_Helper_Simple {
	...
	public function __construct( $product, $actions, $feed_id = '1' ) {
		new IP2Y_Error_Log(
			sprintf( 'FEED № %1$s; %2$s product_id = %3$s, actions = %4$s; Файл: %5$s; Строка: %6$s',
				...
				$product->get_id(),
				$actions,
				'class-ip2y-api-helper-simple.php',
				__LINE__
			)
		);
	...
	public function set_product_stocks() {
		...
		$product_stocks_arr = apply_filters(
			'yp2y_f_simple_product_stocks_arr',
			$product_stocks_arr,
			[ 'product' => $this->get_product() ],
			$this->get_feed_id()
		);
	...
	public function get_stock_amount() {
		if ( false === $this->get_product()->is_in_stock() ) {
			...
			if ( true == $this->get_product()->get_manage_stock() ) { // если включено управление запасом
				return $this->get_product()->get_stock_quantity(); // вернём реальные остатки
	...
	public function product_upd() {
		...
		if ( empty( $shop_sku ) ) {
			$this->add_skip_reason( [ 
				...
				'file' => 'class-ip2y-yandex-api-helper-simple.php',
				'line' => __LINE__
			] );
			return;
		}
		...
		if ( empty( $pictures_arr ) ) {
			$this->add_skip_reason( [ 
				...
				'post_id' => $this->get_product()->get_id(),
				'file' => 'class-ip2y-api-helper-simple.php',
				'line' => __LINE__
			] );
			return;
		}
		...
		$this->result_arr = apply_filters(
			'ip2y_f_simple_helper_result_arr',
			$this->result_arr,
			[ 
				'product' => $this->get_product()
			],
	...
	public function get_shop_sku() {
		...
		if ( empty( $shop_sku ) ) {
			// если у товара вукомерц нет артикула - используем ID товара
		...
		$shop_sku = apply_filters( 'ip2y_f_simple_shop_sku',
			$shop_sku,
			[ 
				'product' => $this->get_product()
			],
	...
	public function get_name() {
		...
		$name = apply_filters( 'ip2y_f_simple_name',
			$name,
			[ 
				'product' => $this->get_product()
			],
	...
	public function get_description() {
		...
		$desc_val = apply_filters( 'ip2y_f_simple_description',
			$desc_val,
			[ 
	...
	public function get_weight_dimensions( $offer_arr ) {
		...
		if ( get_post_meta( $this->get_product()->get_id(), '_ip2y_length', true ) !== '' ) {
			$length = (float) get_post_meta( $this->get_product()->get_id(), '_ip2y_length', true );
		}
		if ( get_post_meta( $this->get_product()->get_id(), '_ip2y_width', true ) !== '' ) {
			$width = (float) get_post_meta( $this->get_product()->get_id(), '_ip2y_width', true );
		}
		if ( get_post_meta( $this->get_product()->get_id(), '_ip2y_height', true ) !== '' ) {
			$height = (float) get_post_meta( $this->get_product()->get_id(), '_ip2y_height', true );
		}
		...
		if ( get_post_meta( $this->get_product()->get_id(), '_ip2y_weight', true ) !== '' ) {
			$weight = (float) get_post_meta( $this->get_product()->get_id(), '_ip2y_weight', true );
			$weight = $weight / 1000;
		}
		if ( ! empty( $weight ) ) {
		...
		$weight_dimensions_arr = apply_filters( 'ip2y_f_simple_weight_dimensions_arr',
			$weight_dimensions_arr,
			[ 
				'product' => $this->get_product()
			],
	...
	public function get_vendor( $offer_arr ) {
		...
		$value = apply_filters( 'ip2y_f_simple_vendor',
			$value,
			[ 
				'product' => $this->get_product(),
				'vybor' => $vybor
	...
	public function get_type( $offer_arr ) {
		$type = 'DEFAULT';
		$type = apply_filters( 'ip2y_f_simple_type',
			$type,
			[ 
				'product' => $this->get_product()
			],
