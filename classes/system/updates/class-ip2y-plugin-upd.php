...
final class IP2Y_Plugin_Upd {
	...
	public function __construct( $args = [] ) {
		...
		$this->list_plugin_names = apply_filters( 'ip2y_f_list_plugin_names', $this->list_plugin_names, $args );
		do_action_ref_array( 'ip2y_a_plugin_upd', $args );
	...
	public function add_plugin_action_links( $actions, $plugin_file ) {
		...
		switch ( $i ) {
			case "202":
				$message = __( 'License is active', 'wc-import-yandex' );
				$color = 'green';
				break;
			case "402":
				$message = __( 'License expired', 'wc-import-yandex' );
				$color = '#dc3232';
				break;
			case "412":
				$message = __( 'License data is invalid', 'wc-import-yandex' );
				$color = '#dc3232';
				break;
			case "418":
				$message = __(
	...
	public function get_info() {
		...
		switch ( $c ) {
			...
			case "402":
				$message = sprintf(
					'<span style="font-weight: 700;">Import Products to Yandex %1$s:</span> %2$s! %3$s, <a href="https://icopydoc.ru/product/%4$s/?utm_source=%4$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-expired" target="_blank">%5$s</a> (%6$s: <span style="font-weight: 700;">%7$s</span>). %8$s <a href="/wp-admin/admin.php?page=ip2y-import">%9$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expired', 'wc-import-yandex' ),
					__( 'Please', 'wc-import-yandex' ),
					$this->get_slug(),
					__( 'purchase a new license', 'wc-import-yandex' ),
					__( 'to get a discount, use this promo code', 'wc-import-yandex' ),
					$this->list_plugin_names[ $this->get_pref()]['code'],
					__( 'If you have already done this', 'wc-import-yandex' ),
					__( 'enter the new license information here', 'wc-import-yandex' )
				);
				break;
			case "412":
				$message = sprintf( '<span style="font-weight: 700;">Import Products to Yandex %1$s:</span> %2$s! %1$s %3$s. <a href="/wp-admin/admin.php?page=ip2y-import">%4$s</a> %5$s <a href="https://icopydoc.ru/product/%6$s/?utm_source=%6$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-err&utm_term=notice" target="_blank">%7$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License data is invalid', 'wc-import-yandex' ),
					__( 'version features do not work and you can not install updates', 'wc-import-yandex' ),
					__( 'Enter your license information', 'wc-import-yandex' ),
					__( 'or', 'wc-import-yandex' ),
					$this->get_slug(),
					__( 'purchase a new one', 'wc-import-yandex' )
				);
				break;
			case "418":
				$message = sprintf( '<span style="font-weight: 700;">Import Products to Yandex %1$s:</span> %2$s! <a href="/wp-admin/admin.php?page=ip2y-import">%3$s</a> %4$s <a href="https://icopydoc.ru/product/%5$s/?utm_source=%5$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-limit&utm_term=notice" target="_blank">%6$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__(
						'This license cannot be used on this site. The package limit has been exceeded',
						'wc-import-yandex'
		...
		if ( $c !== '0' ) {
			...
			if ( $remaining_days > 0 && $remaining_days < 8 ) {
				$message = sprintf(
					'<span style="font-weight: 700;">Import Products to Yandex %1$s:</span> %2$s <span style="font-weight: 700; color: red;">%3$s</span>. %4$s, <a href="https://icopydoc.ru/product/%5$s/?utm_source=link&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-remaining" target="_blank">%6$s</a> (%7$s: <span style="font-weight: 700;">%8$s</span>). %9$s <a href="/wp-admin/admin.php?page=ip2y-import">%10$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expires in', 'wc-import-yandex' ),
					$this->num_decline( $remaining_days, [ 
						__( 'day', 'wc-import-yandex' ),
						_x( 'days', '2 days', 'wc-import-yandex' ),
						_x( 'days', '5 days', 'wc-import-yandex' )
					]
					),
					__( 'Please', 'wc-import-yandex' ),
					$this->get_slug(),
	 ...
	 *
	 * Примеры вызова:
	 * ip2yp_num_decline($num, 'книга,книги,книг')
	 * ip2yp_num_decline($num, ['книга','книги','книг'])
	 * ip2yp_num_decline($num, 'книга', 'книги', 'книг')
	 * ip2yp_num_decline($num, 'книга', 'книг')
	 *
	...
	private function response_to_api() {
		...
		$api_url = apply_filters( 'ip2y_f_api_url', self::API_URL );
	...
	public function set_update_package( $options ) {
		...
		if ( isset( $options['hook_extra']['plugin'] ) ) {
			if ( $options['hook_extra']['plugin'] === $this->plugin_slug ) {
				...
				$package_url = apply_filters( 'ip2y_f_package_url', $package_url, $options );
