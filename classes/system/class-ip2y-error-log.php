 ...
 * @subpackage              Import products to Yandex Market
 * @since                   0.1.0
 * 
 * @version                 0.1.0 (22-04-2024)
 ...
 *                                      get_option
 *                          constants:  IP2Y_PLUGIN_UPLOADS_DIR_PATH
 *                          options:    ip2y_keeplogs
 */
defined( 'ABSPATH' ) || exit;

final class IP2Y_Error_Log {
	/**
	 * Text to log
	 * @var 
	 */
	 ...
	 * @var 
	 */
	protected $log_file_path; // /home/site.ru/public_html/wp-content/uploads/import-products-to-yandex/plugin.log
	 ...
	 * @return void
	 */
	public function __construct( $text_to_log, $log_dir_name = IP2Y_PLUGIN_UPLOADS_DIR_PATH ) {
		...
		if ( is_dir( $log_dir_name ) ) {
			...
			if ( mkdir( $log_dir_name ) ) {
				...
				error_log( 'ERROR: IP2Y_Error_Log: No folder "' . $log_dir_name . '"; Line: ' . __LINE__, 0 );
			}
		}
	...
	protected function keeplogs_status() {
		if ( is_multisite() ) {
			$v = get_blog_option( get_current_blog_id(), 'ip2y_keeplogs' );
		} else {
			$v = get_option( 'ip2y_keeplogs' );
		}
		if ( $v === 'on' ) {
			return true;
		} else {
